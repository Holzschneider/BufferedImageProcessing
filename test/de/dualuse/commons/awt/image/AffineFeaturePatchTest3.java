...
public class AffineFeaturePatchTest3 extends JMicroscope {
	private static final long serialVersionUID = 1L;

	public AffineFeaturePatchTest3(AffineTransform externalCanvasTransform) {
...
//				if ((e.getModifiers()&MouseWheelEvent.ALT_MASK)==0)
	...
	static Sticky<URL> source = new Sticky<URL>( AffineFeaturePatchTest3.class.getResource("frame_0088.jpg") );
	static Sticky<URL> probed = new Sticky<URL>( AffineFeaturePatchTest3.class.getResource("frame_0089-sheared2.jpg") );
	
	static Sticky<Double> radius = new Sticky<Double>( 16. );
	static RGBAffineFeaturePatch afp = new RGBAffineFeaturePatch(radius.get().intValue());
			...
			public void run() {
				...
				synchronized(trail) {
					...
					for (int i=0;i<3*20;i++) try {
						afp
						.reset()
						.track(	from, pbi.width, pbi.height, pbi.pixels, pbi.offset, pbi.scan, 
								over, pbj.width, pbj.height, pbj.pixels, pbj.offset, pbj.scan );
...
//					g.drawImage(new LumaBufferedImage(size, size, afp.J, 0, size), 0, size+1, this);
					g.drawImage(new RGBBufferedImage(size, size, new IntBufferedImage(size,size,afp.G,0,size), new IntBufferedImage(size,size,afp.H,0,size), new IntBufferedImage(size,size,afp.I,0,size)),0,0,this);
					g.drawImage(new RGBBufferedImage(size, size, new IntBufferedImage(size,size,afp.J,0,size), new IntBufferedImage(size,size,afp.K,0,size), new IntBufferedImage(size,size,afp.L,0,size)),0,size+1,this);
				}
			}
		...
		g.setContentPane(new AffineFeaturePatchTest3(Sticky.value(new AffineTransform())) {
...
//				g3.draw(new Line2D.Double(c.getCenterX(), c.getCenterY(), b.getCenterX(), b.getCenterY()));
		...
		f.setContentPane(new AffineFeaturePatchTest3(Sticky.value(new AffineTransform())) {
			private static final long serialVersionUID = 1L;		
